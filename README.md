...
* The local model generates the soft labels of proxy dataset and sends them to the server 
	$ \[p_k^{(1)}, p_k^{(2)}, ..., p_k^{(K)} \] = \[softmax(f_1 (\hat{x_i})), softmax(f_2 (\hat{x_i})), ..., softmax(f_K (\hat{x_i}))\],  i=1,2,...,n $; 
* The server aggregates prediction results and sends them to the clients 
	$ \[ \hat{y_1}, \hat{y_2}, ..., \hat{y_n} \] = \[ ∑_{k=1}^K 1_{\[y_i=k\]} ⋅p_k^{(1)}, _{k=1}^K 1_{\[y_i=k\]} ⋅p_k^{(2)}, ..., _{k=1}^K 1_{\[y_i=k\]} ⋅p_k^{(K)} \]$; 
